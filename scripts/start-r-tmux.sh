...
SERVER_SCRIPT="$ROOT_DIR/backend/tmux_server.py"
...
Start a new tmux-based R session.
...
EXAMPLES:
    ...
    $0 --resume-from r-01f25d24-8464-3440-8000-a35805b91c31 --description "Continue analysis"
    $0 --json
    $0 -p 8080
EOF
}
...
ensure_python_package() {
  ...
  if ! python3 -c "import ${package_name}" 2>/dev/null; then
    echo "missing required Python package: ${package_name}" >&2
    exit 1
  fi
}
...
check_backend_packages() {
  if ! python3 -c "import libtmux" 2>/dev/null; then
    echo "missing required Python package: libtmux" >&2
    echo "Install with: pip install libtmux" >&2
    exit 1
  fi
}
...
TMUX_SESSION_NAME="lcr-$SESSION_ID"
...
python3 - "$MANIFEST_PATH" "$SESSION_ID" "$PORT" "$SESSION_DIR" "$LOG_PATH" "$CHECKPOINT_PATH" "$ARTIFACT_DIR" "$TASK_ID" "$TASK_DESCRIPTION" "$RELATED_FILES_JSON" "$RESUME_FROM" "$RESUMED_FROM_CHECKPOINT" "$TMUX_SESSION_NAME" <<'PY'
import json
import os
import sys
from datetime import datetime, timezone

manifest_path, session_id, port, session_dir, log_path, checkpoint_path, artifact_dir, task_id, task_description, related_files_json, resume_from, resumed_from_checkpoint, tmux_session_name = sys.argv[1:]
...
PY
  ...
  echo ""
  echo "Attach to REPL with: tmux attach -t $TMUX_SESSION_NAME"
fi
